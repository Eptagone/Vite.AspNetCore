...
) : IDisposable
{
    ...
    public void LaunchIfNotRunning()
    {
        ...
        if (!this.isRunning)
        {
            logger.LogViteDevServerDidNotStart(this.options.Server.TimeOut);
        }
        else if (this.options.Server.AutoRun)
        {
            logger.LogViteDevServerRunning(this.options.GetViteDevServerUrl());
        }
    }
    ...
    private async Task<bool> IsViteDevelopmentServerRunning()
    {
        ...
        var httpClient = httpClientFactory.CreateClient(ViteDevServerMiddleware.HTTP_CLIENT_NAME);
    ...
    private async Task StartViteDevServerIfNotRunningAsync()
    {
        ...
        {
            logger.LogViteDevServerAlreadyRunning(this.options.GetViteDevServerUrl());
            return;
        }
        ...
        try
        {
            ...
            {
                logger.LogViteDevServerStarted(this.process.Id);
                ...
                {
                    logger.LogFailedToLaunchStopScript(this.process.Id);
                }
            }
        }
        catch (Exception exp)
    ...
    /// <summary>
    /// On macOS, kill the process tree using a Bash script.
    /// </summary>
    ...
    private static bool LaunchStopScriptForMacOs(int processId)
    {
        ...
        const string fileName = "start-vite-dev-server.sh";
