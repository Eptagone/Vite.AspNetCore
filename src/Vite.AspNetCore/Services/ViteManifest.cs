        ...
        if (viteDevServer.IsEnabled)
        {
            if (warnAboutManifestOnce)
            {
                logger.LogInformation(
                    "The manifest file won't be read because the vite development service is enabled. The service will always return null chunks"
                );
        ...
        get
        {
            if (this.devServerStatus.IsEnabled)
            {
                this.logger.LogWarning(
                    "Attempted to get a record from the manifest file while the vite development server is enabled. Null was returned"
                );
                return null;
            }
            ...
            if (!string.IsNullOrEmpty(this.basePath))
            {
                ...
                {
                    this.logger.LogWarning(
                        "Requesting a chunk with the base path included is deprecated. Please remove the base path from the key '{Key}'",
                        key
                    );
