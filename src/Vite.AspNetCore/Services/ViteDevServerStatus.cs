...
class ViteDevServerStatus : IViteDevServerStatus, IDisposable
{
    ...
    public ViteDevServerStatus(
        ILogger<ViteDevServerStatus> logger,
    ...
    )
    {
        ...
        this.serverUrlWithBasePath = string.IsNullOrEmpty(basePath)
            ? this.serverUrl
            : $"{this.serverUrl}/{basePath}";
        this.logger = logger;
        ...
        {
            logger.LogWarning(
                "The Vite development server did not start within {TotalSeconds} seconds",
                options.Value.Server.TimeOut
            );
        }
        else if (options.Value.Server.AutoRun)
        {
            logger.LogInformation(
                "The Vite development server is running at {ServerUrl}",
                this.serverUrl
            );
        }
    }
    ...
    private async Task StartViteDevServerIfNotRunningAsync()
    {
        ...
        if (await this.IsViteDevelopmentServerRunning())
        {
            this.logger.LogInformation(
                "Looks like the Vite development server is already running at {Url}.",
                this.serverUrl
            );
            return;
        }
        ...
        try
        {
            ...
            {
                this.logger.LogInformation(
                    "Vite development server started with process ID {ProcessId}.",
                    this.process.Id
                );
                ...
                if (stopScriptLaunched == false)
                {
                    this.logger.LogError(
                        "Failed to launch stop script for Vite development server with process ID {ProcessId}.",
                        this.process.Id
                    );
                }
            }
